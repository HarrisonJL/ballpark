...
Ballpark is the fix, built as its own standalone, reusable primitive rather than another one-off contract: a numeric oracle whose equivalence check is a **relative tolerance band**, not exact equality.
...
- `tolerance_bps`: how far apart two independent extractions of the *same* metric can be while still counting as agreement, in basis points of the leader's value (500 = 5%). `0` degenerates to exact match, so this subsumes `strict_eq` as a special case rather than replacing it with something incompatible.
...
`tests/test_ballpark.py` (20 tests, `genlayer-test` Direct Mode) is split into two genuinely different layers:

1. **Integration tests** - real `ask()` calls with a mocked LLM: state bookkeeping, input validation, metric-name deduplication, null-handling for ungrounded extractions.
2. **Consensus-boundary tests** - via gltest's `direct_vm.run_validator(leader_result=...)` cheatcode. Direct Mode runs `leader_fn` directly and only *captures* `validator_fn` for later inspection (it doesn't simulate a real multi-validator vote in-process), so mocking the LLM alone can't exercise disagreement - both "sides" would hit the same canned response and trivially agree. `run_validator` re-invokes the real, captured `validator_fn` closure with an explicit hypothetical leader result, which is the documented, intended way to test this - not a workaround. This is what actually exercises the tolerance math: exact-boundary agreement, one-bps-past-boundary disagreement, the zero-value absolute-band fallback, null-vs-value mismatches, and partial-vector disagreement, all against the *real* validator function that runs in production, not a duplicated copy of the logic.
...
- **Tolerance is per-query, chosen by the caller, and not independently verified against the metric's real-world scale.** A caller requesting a 50% tolerance on a headcount figure will get consensus far more easily than the number is actually useful for - Ballpark enforces the *mechanism* correctly, not whether a given tolerance is a sensible choice for a given use case.
